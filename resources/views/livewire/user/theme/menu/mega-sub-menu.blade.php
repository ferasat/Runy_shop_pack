<ul id="main-mega-menu" class="sub-menu nav">

    <div class="d-flex align-items-start">
        <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
            <button class="nav-link text-danger w160px active" id="Office-machines-tab" data-bs-toggle="pill"
                    data-bs-target="#Office-machines" type="button" role="tab" aria-controls="Office-machines"
                    aria-selected="true">ماشین های اداری
            </button>
            <button class="nav-link text-danger w160px" id="Commodities-tab" data-bs-toggle="pill"
                    data-bs-target="#Commodities" type="button" role="tab" aria-controls="Commodities"
                    aria-selected="false">کالاهای مصرفی
            </button>
            <button class="nav-link text-danger w160px" id="Stationery-tab" data-bs-toggle="pill"
                    data-bs-target="#Stationery" type="button" role="tab" aria-controls="Stationery"
                    aria-selected="false">لواز التحریر
            </button>
            <button class="nav-link text-danger w160px" id="Shop-and-bank-equipment-tab" data-bs-toggle="pill"
                    data-bs-target="#Shop-and-bank-equipment" type="button" role="tab"
                    aria-controls="Shop-and-bank-equipment" aria-selected="false">تجهیزات فروشگاهی و بانکی
            </button>
            <button class="nav-link text-danger w160px" id="Office-medical-equipment-tab" data-bs-toggle="pill"
                    data-bs-target="#Office-medical-equipment" type="button" role="tab"
                    aria-controls="Office-medical-equipment" aria-selected="false">تجهیزات اداری-پزشکی
            </button>
        </div>
        <div class="tab-content w-100" id="v-pills-tabContent">
            <div class="tab-pane fade show active" id="Office-machines" role="tabpanel"
                 aria-labelledby="Office-machines-tab" tabindex="0">
                <!--       ماشین های ادرای         -->
                <div class="row">
                    <div class="col-sm-4 col-md-3">
                        <ol class="list-group ">
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/printer"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" viewBox="0 0 24 24" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                               stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <path d="M9 10H6" stroke="#fc454a" stroke-width="1.5"
                                                      stroke-linecap="round"></path>
                                                <path d="M19 14L5 14" stroke="#fc454a" stroke-width="1.5"
                                                      stroke-linecap="round"></path>
                                                <circle cx="17" cy="10" r="1" fill="#fc454a"></circle>
                                                <path d="M15 16.5H9" stroke="#fc454a" stroke-width="1.5"
                                                      stroke-linecap="round"></path>
                                                <path d="M13 19H9" stroke="#fc454a" stroke-width="1.5"
                                                      stroke-linecap="round"></path>
                                                <path
                                                    d="M22 12C22 14.8284 22 16.2426 21.1213 17.1213C20.48 17.7626 19.5535 17.9359 18 17.9827M6 17.9827C4.44655 17.9359 3.51998 17.7626 2.87868 17.1213C2 16.2426 2 14.8284 2 12C2 9.17157 2 7.75736 2.87868 6.87868C3.75736 6 5.17157 6 8 6H16C18.8284 6 20.2426 6 21.1213 6.87868C21.4211 7.17848 21.6186 7.54062 21.7487 8"
                                                    stroke="#fc454a" stroke-width="1.5" stroke-linecap="round"></path>
                                                <path
                                                    d="M17.9827 6C17.9359 4.44655 17.7626 3.51998 17.1213 2.87868C16.2426 2 14.8284 2 12 2C9.17157 2 7.75736 2 6.87868 2.87868C6.23738 3.51998 6.06413 4.44655 6.01732 6M18 15V16C18 18.8284 18 20.2426 17.1213 21.1213C16.48 21.7626 15.5535 21.9359 14 21.9827M6 15V16C6 18.8284 6 20.2426 6.87868 21.1213C7.51998 21.7626 8.44655 21.9359 10 21.9827"
                                                    stroke="#fc454a" stroke-width="1.5" stroke-linecap="round"></path>
                                            </g>
                                        </svg>
                                        پرینتر
                                    </a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Laser-printer">پرینتر
                                                لیزری</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Ink-jet-printer">پرینتر
                                                جوهر افشان</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Photo-printer">پرینتر
                                                چاپ عکس</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/dot-matrix-printer">پرینتر
                                                سوزنی</a>
                                        </li>
                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/scaner">
                                    <svg class="icon-svg-panel-sm" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                           stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path
                                                d="M7 18.0001C7 18.5523 6.55228 19.0001 6 19.0001C5.44772 19.0001 5 18.5523 5 18.0001C5 17.4478 5.44772 17.0001 6 17.0001C6.55228 17.0001 7 17.4478 7 18.0001Z"
                                                fill="#fc454a"></path>
                                            <path fill-rule="evenodd" clip-rule="evenodd"
                                                  d="M6.6 2.20006C6.15817 1.86869 5.53137 1.95823 5.2 2.40006C4.86863 2.84189 4.95817 3.46869 5.4 3.80006L19 14.0001H4C2.34315 14.0001 1 15.3432 1 17.0001V19.0001C1 20.6569 2.34315 22.0001 4 22.0001H20C21.6569 22.0001 23 20.6569 23 19.0001V16C23 15.0557 22.5604 14.1703 21.8049 13.6037C18.4007 11.0506 8.75927 3.81951 6.6 2.20006ZM21 17.0001C21 16.4478 20.5523 16.0001 20 16.0001H4C3.44772 16.0001 3 16.4478 3 17.0001V19.0001C3 19.5523 3.44772 20.0001 4 20.0001H20C20.5523 20.0001 21 19.5523 21 19.0001V17.0001Z"
                                                  fill="#fc454a"></path>
                                        </g>
                                    </svg>
                                    اسکنر</a>
                            </li>

                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/%D9%84%DB%8C%D8%A8%D9%84-%D8%B2%D9%86"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1"
                                             xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000">
                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                               stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier"><title>label-filled</title>
                                                <g id="Page-1" stroke="none" stroke-width="1" fill="none"
                                                   fill-rule="evenodd">
                                                    <g id="icon" fill="#fc454a"
                                                       transform="translate(21.333333, 106.666667)">
                                                        <path
                                                            d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M362.666667,170.666667 L213.333333,170.666667 L213.333333,213.333333 L362.666667,213.333333 L362.666667,170.666667 Z M117.333333,117.333333 C99.6602213,117.333333 85.3333333,131.660221 85.3333333,149.333333 C85.3333333,167.006445 99.6602213,181.333333 117.333333,181.333333 C135.006445,181.333333 149.333333,167.006445 149.333333,149.333333 C149.333333,131.660221 135.006445,117.333333 117.333333,117.333333 Z M362.666667,85.3333333 L213.333333,85.3333333 L213.333333,128 L362.666667,128 L362.666667,85.3333333 Z"
                                                            id="Combined-Shape"></path>
                                                    </g>
                                                </g>
                                            </g>
                                        </svg>
                                        لیبل زن</a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Label-printer-accessories">لوازم
                                                جانبی لیبل پرینتر</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Manual-labeler">لیبل زن
                                                دستی</a>
                                        </li>

                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/%D9%84%DB%8C%D8%A8%D9%84-%D8%B2%D9%86"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" fill="#fc454a" version="1.1" id="Layer_1"
                                             xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512"
                                             xml:space="preserve" transform="matrix(-1, 0, 0, 1, 0, 0)"><g
                                                id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                               stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <g>
                                                    <g>
                                                        <path
                                                            d="M240.905,102.63C134.963,77.406,59.887,96.486,24.977,122.743C9.523,134.366,0.657,148.207,0,162.133 c0,19.823,16.521,39.057,40.354,48.154C16.572,283.631,3.934,390.972,0,469.333C0,493.261,26.24,512,59.733,512h85.333 c4.71,0,8.533-3.814,8.533-8.533v-34.133c0-4.719-3.823-8.533-8.533-8.533c-18.586,0-33.758-18.671-34.125-41.788l25.404-118.554 c0.589-2.748-0.213-5.615-2.142-7.654c-0.802-0.845-1.365-1.783-2.065-2.671H153.6c2.799,0,5.419-1.374,7.014-3.678 c1.596-2.304,1.954-5.231,0.981-7.851c-3.661-9.771-0.265-37.905,17.084-60.262l5.239,2.62c1.826,0.896,3.9,1.143,5.888,0.64 l34.133-8.533c3.251-0.811,5.726-3.448,6.323-6.75l17.067-93.867C248.141,107.998,245.325,103.68,240.905,102.63z M35.234,136.388 c21.026-15.812,58.573-28.621,110.549-28.621c10.598,0,21.914,0.657,33.69,1.809c-0.077,0.461-0.273,0.87-0.273,1.357 c0,23.125-31.266,42.667-68.267,42.667H19.695C22.639,147.823,27.947,141.867,35.234,136.388z M94.054,416.341 c-0.128,0.589-0.188,1.195-0.188,1.792c0,29.542,18.483,54.153,42.667,58.897v17.903h-76.8c-23.125,0-42.667-11.725-42.675-25.173 c0.29-5.692,0.631-11.571,1.016-17.544c13.705-0.29,26.863-6.443,37.137-17.476c16.213-17.417,23.876-44.74,21.589-76.663 c-0.555-14.14,4.344-27.332,13.79-37.154c7.561-7.868,17.664-12.902,26.829-13.576L94.054,416.341z M143.369,273.067h-18.645 c-1.647-10.513,0.708-21.47,4.736-30.216c6.912-14.985,18.534-25.284,31.147-28.442 C147.337,234.539,142.327,257.246,143.369,273.067z M171.102,196.275c-23.066-1.289-45.551,14.302-57.131,39.433 c-8.823,19.14-9.037,38.955-1.425,55.014c-12.262,1.843-24.67,8.38-34.261,18.355c-12.689,13.201-19.277,30.831-18.526,49.929 c1.963,27.426-4.087,50.185-17.041,64.094c-6.682,7.177-14.942,11.213-23.441,11.844c5.692-74.931,18.449-165.615,39.979-227.337 c0.785-2.253,0.597-4.727-0.529-6.835c-1.126-2.099-3.081-3.644-5.393-4.233c-16.64-4.301-29.833-14.746-34.415-25.873h92.015 c27.793,0,52.343,11.025,62.848,26.317C172.919,196.642,172.032,196.326,171.102,196.275z M214.451,197.862l-19.627,4.907 c-4.275-17.365-18.765-31.974-39.322-40.636c24.363-10.257,40.363-28.834,40.695-50.526c10.59,1.493,21.453,3.311,32.87,5.82 L214.451,197.862z"></path>
                                                    </g>
                                                </g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="M452.267,0c-4.71,0-8.533,3.814-8.533,8.533s3.823,8.533,8.533,8.533c17.818,0,42.667,55.194,42.667,145.067 S470.084,307.2,452.267,307.2c-4.71,0-8.533,3.814-8.533,8.533s3.823,8.533,8.533,8.533c38.801,0,59.733-83.533,59.733-162.133 S491.068,0,452.267,0z"></path>
                                                    </g>
                                                </g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="M384,34.133c-4.71,0-8.533,3.814-8.533,8.533S379.29,51.2,384,51.2c20.181,0,42.667,45.559,42.667,110.933 S404.181,273.067,384,273.067c-4.71,0-8.533,3.814-8.533,8.533c0,4.719,3.823,8.533,8.533,8.533 c34.057,0,59.733-55.023,59.733-128S418.057,34.133,384,34.133z"></path>
                                                    </g>
                                                </g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="M315.733,68.267c-4.71,0-8.533,3.814-8.533,8.533s3.823,8.533,8.533,8.533c23.125,0,42.667,35.166,42.667,76.8 c0,41.634-19.541,76.8-42.667,76.8c-4.71,0-8.533,3.814-8.533,8.533s3.823,8.533,8.533,8.533 c33.493,0,59.733-41.233,59.733-93.867S349.227,68.267,315.733,68.267z"></path>
                                                    </g>
                                                </g>
                                            </g></svg>
                                        بارکدخوان</a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Wired-barcode-reader">بارکد
                                                خوان سیم دار</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Wireless-barcode-reader">بارکد
                                                خوان بی سیم</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Mobile-barcode-reader">بارکد
                                                خوان همراه</a>
                                        </li>

                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>

                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <a class="fw-bold text-danger"
                                   href="https://mortazavistore.ir/product-category/Shredder">
                                    <svg class="icon-svg-panel-sm" fill="#fc454a" version="1.1" id="Layer_1"
                                         xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                         viewBox="0 0 512 512" xml:space="preserve"><g id="SVGRepo_bgCarrier"
                                                                                       stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                           stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <g>
                                                <g>
                                                    <path
                                                        d="M256,75.453h-21.558c-4.466,0-8.084,3.619-8.084,8.084s3.618,8.084,8.084,8.084H256c4.466,0,8.084-3.619,8.084-8.084 S260.466,75.453,256,75.453z"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M363.789,75.453h-75.453c-4.466,0-8.084,3.619-8.084,8.084s3.618,8.084,8.084,8.084h75.453 c4.466,0,8.084-3.619,8.084-8.084S368.255,75.453,363.789,75.453z"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M234.442,118.568h-64.674c-4.466,0-8.084,3.619-8.084,8.084s3.619,8.084,8.084,8.084h64.674 c4.466,0,8.084-3.619,8.084-8.084S238.908,118.568,234.442,118.568z"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M363.789,118.568h-97.011c-4.466,0-8.084,3.619-8.084,8.084s3.618,8.084,8.084,8.084h97.011 c4.466,0,8.084-3.619,8.084-8.084S368.255,118.568,363.789,118.568z"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M105.095,247.916c-16.344,0-29.642,13.298-29.642,29.642S88.751,307.2,105.095,307.2s29.642-13.298,29.642-29.642 S121.439,247.916,105.095,247.916z M105.095,291.032c-7.43,0-13.474-6.044-13.474-13.474s6.044-13.474,13.474-13.474 s13.474,6.044,13.474,13.474S112.525,291.032,105.095,291.032z"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M192.233,258.695h-23.371c-9.902,0-17.957,8.056-17.957,17.957v12.592c0,9.902,8.056,17.957,17.957,17.957h23.371 c9.902,0,17.957-8.056,17.957-17.957v-12.592C210.189,266.751,202.133,258.695,192.233,258.695z M194.021,289.243 c0,0.986-0.802,1.788-1.788,1.788h-23.371c-0.986,0-1.788-0.802-1.788-1.788v-12.592c0-0.986,0.802-1.788,1.788-1.788h23.371 c0.986,0,1.788,0.802,1.788,1.788V289.243z"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M462.295,339.537c27.407,0,49.705-22.298,49.705-49.705V179.052c0-27.407-22.298-49.705-49.705-49.705h-58.085V42.547 C404.211,19.086,385.124,0,361.664,0H205.601c-11.364,0-22.048,4.426-30.085,12.464L120.251,67.73 c-8.037,8.037-12.462,18.721-12.462,30.085v31.533H49.705C22.298,129.347,0,151.646,0,179.052v110.778 c0,27.408,22.298,49.706,49.705,49.706h25.748v78.147c0,19.317-15.715,35.032-35.032,35.032H8.084 C3.619,452.716,0,456.334,0,460.8v43.116C0,508.382,3.619,512,8.084,512h32.337c41.655,0,77.085-27.146,89.54-64.674h31.724 v56.589c0,4.466,3.619,8.084,8.084,8.084h43.116c4.466,0,8.084-3.618,8.084-8.084v-56.589h26.947v56.589 c0,4.466,3.619,8.084,8.084,8.084h43.116c4.466,0,8.084-3.618,8.084-8.084v-56.589h26.947v56.589c0,4.466,3.619,8.084,8.084,8.084 h43.116c4.466,0,8.084-3.618,8.084-8.084v-33.479C410.403,495.497,439.1,512,471.579,512h32.337c4.466,0,8.084-3.618,8.084-8.084 V460.8c0-4.466-3.618-8.084-8.084-8.084h-32.337c-19.317,0-35.032-15.715-35.032-35.032v-78.147H462.295z M404.211,188.632h2.695 c7.43,0,13.474,6.044,13.474,13.474s-6.044,13.474-13.474,13.474h-2.695V188.632z M183.242,27.603v47.85h-47.849L183.242,27.603z M118.568,417.684c0,43.091-35.056,78.147-78.147,78.147H16.168v-26.947h24.253c28.231,0,51.2-22.969,51.2-51.2v-78.147h26.947 V417.684z M204.8,495.832h-26.947V374.568c0-4.466-3.619-8.084-8.084-8.084s-8.084,3.619-8.084,8.084v56.589h-26.947v-91.621 H204.8V495.832z M291.032,495.832h-26.947V374.568c0-4.466-3.619-8.084-8.084-8.084c-4.466,0-8.084,3.619-8.084,8.084v56.589 h-26.947v-91.621h70.063V495.832z M377.263,495.832h-26.947V374.568c0-4.466-3.618-8.084-8.084-8.084 c-4.466,0-8.084,3.619-8.084,8.084v56.589H307.2v-91.621h70.063V495.832z M471.579,468.884h24.253v26.947h-24.253 c-43.091,0-78.147-35.056-78.147-78.147v-78.147h26.947v78.147C420.379,445.915,443.348,468.884,471.579,468.884z M49.705,323.368 c-18.521,0-33.536-15.015-33.536-33.537V179.052c0-18.521,15.015-33.537,33.536-33.537h58.085v26.947h-2.064 c-16.129,0-29.78,12.621-30.26,28.744c-0.498,16.752,12.986,30.54,29.629,30.54h191.537c4.465,0,8.084-3.62,8.084-8.084 c0-4.465-3.62-8.084-8.084-8.084H123.958V97.815c0-2.114,0.252-4.189,0.732-6.194h66.636c4.465,0,8.084-3.62,8.084-8.084V16.9 c2.004-0.48,4.078-0.732,6.19-0.732h156.063c14.569,0,26.378,11.809,26.378,26.378v173.032h-55.766 c-4.465,0-8.084,3.62-8.084,8.084c0,4.465,3.62,8.084,8.084,8.084h73.999c16.129,0,29.78-12.621,30.26-28.744 c0.498-16.752-12.986-30.54-29.629-30.54h-2.695v-26.947h58.085c18.521,0,33.536,15.015,33.536,33.537v110.778 c0,18.523-15.015,33.538-33.536,33.538H49.705z M107.789,188.632v26.947h-2.695c-7.43,0-13.474-6.044-13.474-13.474 s6.044-13.474,13.474-13.474H107.789z"></path>
                                                </g>
                                            </g>
                                        </g></svg>
                                    کاغذ خردکن
                                </a>
                            </li>

                        </ol>
                    </div>
                    <div class="col-sm-4 col-md-3">
                        <ol class="list-group ">
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/accessories"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" version="1.1" id="Layer_1"
                                             xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32"
                                             enable-background="new 0 0 32 32" xml:space="preserve" fill="#000000"><g
                                                id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                               stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <g>
                                                    <path fill="#fc454a"
                                                          d="M1.836,23.001c-1.042,0.891-1.672,2.188-1.728,3.559c-0.056,1.37,0.467,2.714,1.435,3.687 c0.935,0.939,2.179,1.457,3.504,1.457c1.424,0,2.778-0.613,3.716-1.681l6.626-7.474l7.739,7.739 c0.916,0.916,2.134,1.421,3.429,1.421c1.296,0,2.514-0.505,3.43-1.421s1.421-2.134,1.421-3.43c0-1.295-0.505-2.513-1.421-3.429 l-8.152-8.152l2.042-2.303c0.604,0.172,1.225,0.259,1.848,0.259c1.903,0,3.659-0.817,4.818-2.242c1.601-1.967,1.8-4.746,0.507-7.08 c-0.291-0.524-1.166-0.503-1.443,0.031l-1.473,2.837c-0.354,0.683-1.248,0.967-1.937,0.612l-1.502-0.782 c-0.341-0.176-0.592-0.475-0.707-0.84c-0.116-0.365-0.082-0.754,0.094-1.094l1.66-3.188c0.133-0.256,0.124-0.556-0.025-0.804 c-0.146-0.242-0.401-0.39-0.75-0.392c-1.903,0-3.66,0.816-4.819,2.241c-1.285,1.579-1.679,3.691-1.079,5.712l-5.612,4.806 L5.739,5.334C6.204,4.658,6.346,3.819,6.08,3.012C5.768,2.065,5.002,1.38,4.033,1.177L1.97,0.744 C1.544,0.653,1.1,0.787,0.793,1.095c-0.309,0.309-0.44,0.749-0.351,1.177l0.433,2.063c0.265,1.266,1.396,2.186,2.69,2.186 c0.537,0,1.038-0.166,1.476-0.47l7.653,7.653L1.836,23.001z M3.565,5.538c-0.706,0-1.521-0.46-1.712-1.373L1.421,2.136 C1.401,2.04,1.431,2.01,1.5,1.941C1.55,1.891,1.615,2,1.73,2c0.011,0,0.022,0,0.034,0l2.063,0.295 C4.444,2.424,4.931,2.724,5.13,3.327C5.319,3.9,5.197,4.507,4.811,4.943L4.643,5.116C4.339,5.386,3.966,5.538,3.565,5.538z M29.279,24.137c0.728,0.727,1.128,1.693,1.128,2.722c0,1.029-0.4,1.996-1.128,2.723c-1.452,1.455-3.99,1.455-5.444,0l-7.782-7.782 l5.117-5.772L29.279,24.137z M20.118,8.242c-0.63-1.786-0.329-3.685,0.805-5.078C21.83,2.05,23.18,1.38,24.712,1.3l-1.518,2.914 c-0.299,0.578-0.356,1.237-0.16,1.857s0.623,1.127,1.2,1.426l1.502,0.782c0.347,0.179,0.733,0.273,1.121,0.273 c0.913,0,1.742-0.503,2.163-1.312l1.315-2.533c0.913,1.905,0.703,4.091-0.568,5.653c-1.347,1.654-3.753,2.286-5.891,1.567 c-0.19-0.063-0.399-0.008-0.534,0.142L8.013,29.36c-0.749,0.854-1.83,1.343-2.966,1.343c-1.057,0-2.05-0.413-2.795-1.162 c-0.783-0.787-1.189-1.832-1.145-2.941s0.534-2.117,1.379-2.839L19.972,8.788C20.129,8.653,20.187,8.437,20.118,8.242z"></path>
                                                    <path fill="#fc454a"
                                                          d="M19.877,21.033l6.026,6.026c0.098,0.098,0.226,0.146,0.354,0.146s0.256-0.049,0.354-0.146 c0.195-0.195,0.195-0.512,0-0.707l-6.026-6.026c-0.195-0.195-0.512-0.195-0.707,0S19.682,20.838,19.877,21.033z"></path>
                                                    <path fill="#fc454a"
                                                          d="M5.065,24.769c-1.103,0-2,0.897-2,2s0.897,2,2,2s2-0.897,2-2S6.168,24.769,5.065,24.769z M5.065,27.769 c-0.552,0-1-0.448-1-1s0.448-1,1-1s1,0.448,1,1S5.617,27.769,5.065,27.769z"></path>
                                                </g>
                                            </g></svg>
                                        لوازم جانبی
                                    </a>
                                    <ul class="list-group">
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/chipset">چیپست</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link" href="https://mortazavistore.ir/product-category/drama">درام</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Printer-head">هد
                                                پرینتر</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/adapter">آداپتور</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link" href="https://mortazavistore.ir/product-category/blade">بلید</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/magnet">مگنت</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/foam-roller">فوم
                                                رولر</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/fusing">فیوزینگ</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Fusing-movie">فیلم
                                                فیوزینگ</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Foam-meter-board">برد
                                                فومتر</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/power-board">برد
                                                پاور</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Belt-transfer">ترنسفر
                                                بلت</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link" href="https://mortazavistore.ir/product-category/Pres">پرس</a>
                                        </li>
                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>
                        </ol>
                    </div>

                    <div class="col-sm-4 col-md-3">
                        <ol class="list-group ">
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/accessories"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" version="1.1" id="Layer_1"
                                             xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32"
                                             enable-background="new 0 0 32 32" xml:space="preserve" fill="#000000"><g
                                                id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                               stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <g>
                                                    <path fill="#fc454a"
                                                          d="M1.836,23.001c-1.042,0.891-1.672,2.188-1.728,3.559c-0.056,1.37,0.467,2.714,1.435,3.687 c0.935,0.939,2.179,1.457,3.504,1.457c1.424,0,2.778-0.613,3.716-1.681l6.626-7.474l7.739,7.739 c0.916,0.916,2.134,1.421,3.429,1.421c1.296,0,2.514-0.505,3.43-1.421s1.421-2.134,1.421-3.43c0-1.295-0.505-2.513-1.421-3.429 l-8.152-8.152l2.042-2.303c0.604,0.172,1.225,0.259,1.848,0.259c1.903,0,3.659-0.817,4.818-2.242c1.601-1.967,1.8-4.746,0.507-7.08 c-0.291-0.524-1.166-0.503-1.443,0.031l-1.473,2.837c-0.354,0.683-1.248,0.967-1.937,0.612l-1.502-0.782 c-0.341-0.176-0.592-0.475-0.707-0.84c-0.116-0.365-0.082-0.754,0.094-1.094l1.66-3.188c0.133-0.256,0.124-0.556-0.025-0.804 c-0.146-0.242-0.401-0.39-0.75-0.392c-1.903,0-3.66,0.816-4.819,2.241c-1.285,1.579-1.679,3.691-1.079,5.712l-5.612,4.806 L5.739,5.334C6.204,4.658,6.346,3.819,6.08,3.012C5.768,2.065,5.002,1.38,4.033,1.177L1.97,0.744 C1.544,0.653,1.1,0.787,0.793,1.095c-0.309,0.309-0.44,0.749-0.351,1.177l0.433,2.063c0.265,1.266,1.396,2.186,2.69,2.186 c0.537,0,1.038-0.166,1.476-0.47l7.653,7.653L1.836,23.001z M3.565,5.538c-0.706,0-1.521-0.46-1.712-1.373L1.421,2.136 C1.401,2.04,1.431,2.01,1.5,1.941C1.55,1.891,1.615,2,1.73,2c0.011,0,0.022,0,0.034,0l2.063,0.295 C4.444,2.424,4.931,2.724,5.13,3.327C5.319,3.9,5.197,4.507,4.811,4.943L4.643,5.116C4.339,5.386,3.966,5.538,3.565,5.538z M29.279,24.137c0.728,0.727,1.128,1.693,1.128,2.722c0,1.029-0.4,1.996-1.128,2.723c-1.452,1.455-3.99,1.455-5.444,0l-7.782-7.782 l5.117-5.772L29.279,24.137z M20.118,8.242c-0.63-1.786-0.329-3.685,0.805-5.078C21.83,2.05,23.18,1.38,24.712,1.3l-1.518,2.914 c-0.299,0.578-0.356,1.237-0.16,1.857s0.623,1.127,1.2,1.426l1.502,0.782c0.347,0.179,0.733,0.273,1.121,0.273 c0.913,0,1.742-0.503,2.163-1.312l1.315-2.533c0.913,1.905,0.703,4.091-0.568,5.653c-1.347,1.654-3.753,2.286-5.891,1.567 c-0.19-0.063-0.399-0.008-0.534,0.142L8.013,29.36c-0.749,0.854-1.83,1.343-2.966,1.343c-1.057,0-2.05-0.413-2.795-1.162 c-0.783-0.787-1.189-1.832-1.145-2.941s0.534-2.117,1.379-2.839L19.972,8.788C20.129,8.653,20.187,8.437,20.118,8.242z"></path>
                                                    <path fill="#fc454a"
                                                          d="M19.877,21.033l6.026,6.026c0.098,0.098,0.226,0.146,0.354,0.146s0.256-0.049,0.354-0.146 c0.195-0.195,0.195-0.512,0-0.707l-6.026-6.026c-0.195-0.195-0.512-0.195-0.707,0S19.682,20.838,19.877,21.033z"></path>
                                                    <path fill="#fc454a"
                                                          d="M5.065,24.769c-1.103,0-2,0.897-2,2s0.897,2,2,2s2-0.897,2-2S6.168,24.769,5.065,24.769z M5.065,27.769 c-0.552,0-1-0.448-1-1s0.448-1,1-1s1,0.448,1,1S5.617,27.769,5.065,27.769z"></path>
                                                </g>
                                            </g></svg>
                                        لوازم جانبی
                                    </a>
                                    <ul class="list-group">

                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/hot-roll">هات رول</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/paper-cutter">کاغذ
                                                کش</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Suppression-Pad">سپریشن
                                                پد</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/output-tray">سینی
                                                خروجی</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/input-tray">سینی
                                                ورودی</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/pterygium">ناخنک</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link" href="https://mortazavistore.ir/product-category/gear">چرخ
                                                دنده</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Other-parts">سایر
                                                قطعات</a>
                                        </li>
                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>

                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <a class="fw-bold text-danger"
                                   href="https://mortazavistore.ir/product-category/Attendance-device">
                                    <svg class="icon-svg-panel-sm" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                           stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path
                                                d="M8.5 15.7409C8.5 14.7575 9.20862 13.9198 10.1717 13.7648C11.3829 13.5699 12.6171 13.5699 13.8283 13.7648C14.7914 13.9198 15.5 14.7575 15.5 15.7409V15.9586C15.5 16.5338 15.0376 17 14.4673 17H9.53269C8.96235 17 8.5 16.5338 8.5 15.9586V15.7409Z"
                                                fill="#fc454a" fill-opacity="0.15"></path>
                                            <path
                                                d="M14.0417 9.05882C14.0417 10.1959 13.1276 11.1176 12 11.1176C10.8724 11.1176 9.95833 10.1959 9.95833 9.05882C9.95833 7.92177 10.8724 7 12 7C13.1276 7 14.0417 7.92177 14.0417 9.05882Z"
                                                fill="#fc454a" fill-opacity="0.15"></path>
                                            <path
                                                d="M8.5 15.7409C8.5 14.7575 9.20862 13.9198 10.1717 13.7648C11.3829 13.5699 12.6171 13.5699 13.8283 13.7648C14.7914 13.9198 15.5 14.7575 15.5 15.7409V15.9586C15.5 16.5338 15.0376 17 14.4673 17H9.53269C8.96235 17 8.5 16.5338 8.5 15.9586V15.7409Z"
                                                stroke="#fc454a" stroke-width="1.5"></path>
                                            <path
                                                d="M14.0417 9.05882C14.0417 10.1959 13.1276 11.1176 12 11.1176C10.8724 11.1176 9.95833 10.1959 9.95833 9.05882C9.95833 7.92177 10.8724 7 12 7C13.1276 7 14.0417 7.92177 14.0417 9.05882Z"
                                                stroke="#fc454a" stroke-width="1.5"></path>
                                            <path
                                                d="M21 7C21 4.79086 18.7614 3 16 3M3 16C3 18.7614 4.79086 21 7 21M7 3C4.79086 3 3 4.79086 3 7M16 21C18.7614 21 21 18.7614 21 16"
                                                stroke="#fc454a" stroke-width="1.5" stroke-linecap="round"></path>
                                        </g>
                                    </svg>
                                    حضورغیاب</a>
                            </li>

                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/Receipt-printer"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" viewBox="0 0 24 24" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                               stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <rect width="24" height="24" fill="white"></rect>
                                                <path
                                                    d="M18.75 17H20C20.5523 17 21 16.5523 21 16V8C21 7.44772 20.5523 7 20 7H4C3.44772 7 3 7.44772 3 8V16C3 16.5523 3.44772 17 4 17H5.25"
                                                    stroke="#fc454a" stroke-linecap="round"
                                                    stroke-linejoin="round"></path>
                                                <path
                                                    d="M6 12C6 11.4477 6.44772 11 7 11H17C17.5523 11 18 11.4477 18 12V20C18 20.5523 17.5523 21 17 21H7C6.44772 21 6 20.5523 6 20V12Z"
                                                    stroke="#fc454a" stroke-linecap="round"
                                                    stroke-linejoin="round"></path>
                                                <path
                                                    d="M6 4C6 3.44772 6.44772 3 7 3H17C17.5523 3 18 3.44772 18 4V7H6V4Z"
                                                    stroke="#fc454a" stroke-linecap="round"
                                                    stroke-linejoin="round"></path>
                                                <path d="M8.5 13.5H15.5" stroke="#fc454a" stroke-linecap="round"
                                                      stroke-linejoin="round"></path>
                                                <path d="M8.5 18.5H15.5" stroke="#fc454a" stroke-linecap="round"
                                                      stroke-linejoin="round"></path>
                                                <path d="M8.5 16H15.5" stroke="#fc454a" stroke-linecap="round"
                                                      stroke-linejoin="round"></path>
                                            </g>
                                        </svg>
                                        فیش پرینتر</a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Receipt-printer-accessories">لوازم
                                                جانبی فیش پرینتر</a>
                                        </li>
                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>

                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <a class="fw-bold text-danger"
                                   href="https://mortazavistore.ir/product-category/Copying-machin">
                                    <svg class="icon-svg-panel-sm" fill="#fc454a" version="1.1" id="Layer_1"
                                         xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                         viewBox="0 0 503.581 503.581" xml:space="preserve"><g id="SVGRepo_bgCarrier"
                                                                                               stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                           stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="M251.791,167.844c-41.648,0-75.541,33.884-75.541,75.541c0,41.657,33.893,75.541,75.541,75.541 s75.541-33.884,75.541-75.541C327.332,201.728,293.439,167.844,251.791,167.844z M293.758,251.778h-33.574v33.574 c0,4.642-3.76,8.393-8.393,8.393s-8.393-3.752-8.393-8.393v-33.574h-33.574c-4.633,0-8.393-3.752-8.393-8.393 c0-4.642,3.76-8.393,8.393-8.393h33.574v-33.574c0-4.642,3.76-8.393,8.393-8.393s8.393,3.752,8.393,8.393v33.574h33.574 c4.633,0,8.393,3.752,8.393,8.393C302.151,248.026,298.391,251.778,293.758,251.778z"></path>
                                                        <path
                                                            d="M352.512,411.254c23.141,0,41.967-18.827,41.967-41.967v-268.59c0-23.141-18.827-41.967-41.967-41.967h-16.787v-8.318 c0-9.258-7.529-16.787-16.787-16.787h-16.501V25.18c0-6.74-2.627-13.069-7.395-17.819C290.291,2.61,283.98,0,277.265,0h-0.042 l-50.554,0.076c-13.866,0.008-25.147,11.306-25.147,25.18v8.368h-16.779c-9.233,0-16.762,7.512-16.787,16.753l-0.025,8.351 h-16.862c-23.141,0-41.967,18.826-41.967,41.967v268.59c0,23.141,18.826,41.967,41.967,41.967h92.328v25.18h-33.574 c-13.883,0-25.18,11.298-25.18,25.18h-67.148c-4.633,0-8.393,3.752-8.393,8.393c0,4.642,3.76,8.393,8.393,8.393h67.148 c0,13.883,11.298,25.18,25.18,25.18h83.934c13.883,0,25.18-11.298,25.18-25.18h67.148c4.633,0,8.393-3.752,8.393-8.393 c0-4.642-3.76-8.393-8.393-8.393h-67.148c0-13.883-11.298-25.18-25.18-25.18h-33.574v-25.18H352.512z M184.744,50.411h25.172 c4.633,0,8.393-3.752,8.393-8.393V25.256c0-4.625,3.76-8.393,8.385-8.393l50.554-0.076h0.017c2.233,0,4.339,0.865,5.926,2.451 s2.459,3.693,2.459,5.943v16.837c0,4.642,3.76,8.393,8.393,8.393h24.895v16.711v16.795L184.66,84.035L184.744,50.411z M251.791,335.712c-50.906,0-92.328-41.422-92.328-92.328c0-50.906,41.422-92.328,92.328-92.328s92.328,41.422,92.328,92.328 C344.119,294.291,302.697,335.712,251.791,335.712z"></path>
                                                    </g>
                                                </g>
                                            </g>
                                        </g></svg>
                                    دستگاه کپی
                                </a>
                            </li>


                        </ol>
                    </div>

                </div>
            </div>
            <div class="tab-pane fade" id="Commodities" role="tabpanel" aria-labelledby="Commodities-tab" tabindex="0">
                <!--       کالای مصرفی         -->
                <div class="row">
                    <div class="col-sm-4 col-md-3">
                        <ol class="list-group ">
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/cartridge"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" viewBox="0 0 48 48" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                               stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <rect x="5" y="5" width="38" height="20" rx="2" fill="#f99fa2"
                                                      stroke="#fc454a" stroke-width="4" stroke-linecap="round"
                                                      stroke-linejoin="round"></rect>
                                                <path d="M14 5V25" stroke="white" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M34 5V25" stroke="white" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M30 5H38" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M5 11L43 11" stroke="white" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M5 19L43 19" stroke="white" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M5 21V9" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M43 21V9" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M10 5H18" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M30 25H38" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M10 25H18" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M32 25C32 29.4183 28.4183 33 24 33C19.5817 33 16 29.4183 16 25"
                                                      stroke="#fc454a" stroke-width="4" stroke-linecap="round"
                                                      stroke-linejoin="round"></path>
                                                <path d="M28 32L27 37V44" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                                <path d="M20 32L21 37V44" stroke="#fc454a" stroke-width="4"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                            </g>
                                        </svg>
                                        کارتریج
                                    </a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Color-laser-cartridge">کارتریج
                                                لیزری رنگی</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Black-laser-cartridge">کارتریج
                                                لیزری مشکی</a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Inkjet-cartridge">کارتریج
                                                جوهر افشان</a>
                                        </li>

                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>

                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/ribbon"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M53.3506 238.839C53.4096 238.617 79.7163 201.805 79.7163 200.388C79.7163 199.005 50.9893 163.43 51 163.186C51.0161 162.771 51.0625 162.355 51.1095 161.94C51.1313 161.732 51.1376 161.522 51.1572 161.313C52.7645 161.273 54.383 161.261 55.9896 161.214C57.8158 161.161 59.6385 161.099 61.462 161.047C65.1355 160.939 68.807 161.041 72.4721 161.116C76.1738 161.192 79.8691 161.238 83.5763 161.257C87.4773 161.275 91.3817 161.287 95.2827 161.257C102.491 161.202 131.676 161.334 133.474 161.298C135.152 161.265 136.822 161.226 138.502 161.213C141.932 161.187 333.699 161.186 337.129 161.215C339.158 161.231 341.183 161.237 343.213 161.236C344.12 161.236 345.03 161.228 345.941 161.217C346.256 161.213 346.572 161.205 346.889 161.198C346.979 161.351 347.057 161.505 347.144 161.654C347.267 161.864 347.367 162.074 347.464 162.284C347.645 162.874 318.473 199.655 318.598 200.947C318.713 202.159 349.48 235.357 349.694 236.167C349.851 236.776 350.026 237.615 349.941 238.227C348.193 238.288 346.555 238.301 344.91 238.334C344.011 238.352 288.127 236.052 229.098 236.167C156.344 236.309 77.5561 238.874 75.8919 238.913C74.3779 238.95 72.8647 238.984 71.3487 239" stroke="#fc454a" stroke-opacity="0.9" stroke-width="16" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                                        ریبون
                                    </a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Needle-printer-ribbon">
                                                ریبون پرینتر سوزنی
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Ribbon-label-printer">
                                                ریبون لیبل پرینتر
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Card-printer-ribbon">
                                                ریبون کارت پرینتر
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Historian-ribbon">
                                                ریبون تاریخ زن
                                            </a>
                                        </li>

                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>

                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Toner">
                                    <svg class="icon-svg-panel-sm" viewBox="0 0 32 32" data-name="Layer 1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><defs><style>.cls-1{fill:none;stroke:#fc454a;stroke-linejoin:round;}</style></defs><title></title><path class="cls-1" d="M20.5,10.56V27.5a1,1,0,0,1-1,1h-7a1,1,0,0,1-1-1V10.56a3.05,3.05,0,0,1,2-2.87,3.11,3.11,0,0,1,1.06-.19h2.88a3.11,3.11,0,0,1,1.06.19A3.05,3.05,0,0,1,20.5,10.56Z"></path><path class="cls-1" d="M18.5,3.5V7.69a3.11,3.11,0,0,0-1.06-.19H14.56a3.11,3.11,0,0,0-1.06.19V3.5Z"></path></g></svg>
                                    تونر
                                </a>
                            </li>

                        </ol>
                    </div>
                    <div class="col-sm-4 col-md-3">
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/label">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                لیبل (برچسب)
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Printer-label-tape">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                نوار لیبل پرینتر
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Manual-tape-labeller">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                نوار لیبل زن دستی
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/PVC-card">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                کارت PVC
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Thermal-roll">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                رول حرارتی
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Printer-and-plotter-ink">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                جوهر پرینتر و پلاتر
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Rechargeable-tank-and-cartridge">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                تانک و کارتریج قابل شارژ
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/photo-paper">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                کاغذ عکس
                            </a>
                        </li>
                    </div>
                    <div class="col-sm-4 col-md-3">
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/developer">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                دولوپر
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Risograph-compound">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                مرکب ریسوگراف
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/battery">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M11 9L9.5 12H13.5L12 15M21 13V11M6.2 18H16.8C17.9201 18 18.4802 18 18.908 17.782C19.2843 17.5903 19.5903 17.2843 19.782 16.908C20 16.4802 20 15.9201 20 14.8V9.2C20 8.0799 20 7.51984 19.782 7.09202C19.5903 6.71569 19.2843 6.40973 18.908 6.21799C18.4802 6 17.9201 6 16.8 6H6.2C5.0799 6 4.51984 6 4.09202 6.21799C3.71569 6.40973 3.40973 6.71569 3.21799 7.09202C3 7.51984 3 8.07989 3 9.2V14.8C3 15.9201 3 16.4802 3.21799 16.908C3.40973 17.2843 3.71569 17.5903 4.09202 17.782C4.51984 18 5.07989 18 6.2 18Z" stroke="#fc454a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                                باتری
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <a class="fw-bold text-danger" href="https://mortazavistore.ir/product-category/Carbon-Fix">
                                <svg class="icon-svg-panel-sm" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>label</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="icon" fill="#fc454a" transform="translate(21.333333, 106.666667)"> <path d="M448,2.84217094e-14 L448,298.666667 L106.666667,298.666667 L3.55271368e-15,149.333333 L106.666667,2.84217094e-14 L448,2.84217094e-14 Z M405.333333,42.6666667 L128.597333,42.6666667 L52.416,149.333333 L128.618667,256 L405.333333,256 L405.333333,42.6666667 Z M138.666667,117.333333 C156.339779,117.333333 170.666667,131.660221 170.666667,149.333333 C170.666667,167.006445 156.339779,181.333333 138.666667,181.333333 C120.993555,181.333333 106.666667,167.006445 106.666667,149.333333 C106.666667,131.660221 120.993555,117.333333 138.666667,117.333333 Z M213.333333,170.666667 L362.666667,170.666667 L362.666667,213.333333 L213.333333,213.333333 L213.333333,170.666667 Z M213.333333,85.3333333 L362.666667,85.3333333 L362.666667,128 L213.333333,128 L213.333333,85.3333333 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>
                                کاربن فکس
                            </a>
                        </li>
                    </div>
                </div>
            </div>
            <div class="tab-pane fade" id="Stationery" role="tabpanel" aria-labelledby="Stationery-tab" tabindex="0">
            <!--      لواز التحریر          -->
                <div class="row">
                    <div class="col-sm-4 col-md-3">
                        <ol class="list-group ">
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/Stationery"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" fill="#fc454a" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 306.095 306.095" xml:space="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <path d="M171.289,225.186l-40.316-69.828c-0.96-1.664-2.735-2.688-4.654-2.688h-7.171v-17.855c0-2.975-2.409-5.375-5.375-5.375 H58.235c-2.969,0-5.375,2.401-5.375,5.375v17.855h-7.171c-1.918,0-3.698,1.024-4.656,2.688L0.717,225.186 c-0.956,1.67-0.956,3.711,0,5.376l40.315,69.826c0.963,1.665,2.738,2.688,4.656,2.688h80.631c1.918,0,3.698-1.022,4.654-2.688 l40.316-69.826C172.25,228.897,172.25,226.851,171.289,225.186z"></path> <path d="M39.415,123.027h93.177c2.966,0,5.375-2.405,5.375-5.375V98.864c0-2.974-2.41-5.375-5.375-5.375H39.415 c-2.968,0-5.375,2.401-5.375,5.375v18.788C34.04,120.618,36.441,123.027,39.415,123.027z"></path> </g> <path d="M301.836,105.302c-2.451-0.714-5.066,0.373-6.273,2.63c-0.226,0.431-0.473,0.872-0.729,1.323 c6.679-16.197,13.034-37.155,10.809-54.578c-2.608-20.476-12.173-27.242-20.615-33.223c-5.606-3.975-10.445-7.402-13.296-15.014 c-0.85-2.27-3.098-3.548-5.423-3.373c-1.149-0.139-2.347,0.009-3.417,0.651c-2.54,1.533-3.36,4.837-1.821,7.381 c0.247,0.415,0.457,0.855,0.693,1.273c0.515,7.118,0.089,26.651-15.729,50.863c-0.478,0.764-10.702,17.353-1.086,33.987 c-2.052,0.56-4.745-1.113-7.56-4.816c-0.966-1.257-2.436-2.032-4.021-2.113c-1.612-0.113-3.129,0.559-4.204,1.722 c-6.81,7.357-15.524,25.882-2.279,45.24c-1.427,1.191-3.485,2.015-11.004-1.457c-2.418-1.118-5.311-0.291-6.765,1.955 c-0.62,0.958-15.135,23.745-9.318,49.189c3.249,14.242,12.206,26.289,26.505,35.927c-0.379,1.586-0.867,3.14-1.197,4.741 c-8.105,39.291,0.977,70.314,1.365,71.617c0.697,2.33,2.829,3.831,5.149,3.831c0.51,0,1.029-0.072,1.549-0.226 c2.839-0.854,4.456-3.852,3.602-6.692c-0.075-0.278-8.689-29.686-1.124-66.341c0.252-1.224,0.63-2.399,0.908-3.607 c10.058-0.104,41.77-3.974,57.597-46.705c0.798-2.146,0.141-4.565-1.628-6.02c-1.769-1.449-4.269-1.618-6.231-0.426 c-0.016,0.01-0.252,0.158-0.687,0.404c5.517-6.708,10.787-14.719,13.107-22.861c0.199-0.635,4.902-15.843,6.944-39.678 C305.888,108.373,304.282,106.029,301.836,105.302z M210.308,184.656c-3.581-15.523,2.361-30.442,5.747-37.232 c2.883,0.948,5.561,1.42,7.996,1.42c9.879,0,13.722-7.385,14.372-11.299c0.246-1.494-0.142-3.027-1.086-4.212 c-9.658-12.208-6.651-23.153-3.507-29.164c3.239,2.614,6.667,3.935,10.24,3.935c7.497-0.005,12.037-5.89,12.53-6.556 c1.455-1.979,1.387-4.688-0.161-6.591c-9.765-11.956-1.644-25.431-1.345-25.911c8.142-12.465,12.589-23.838,14.992-33.369 c9.408,49.826-18.897,120.396-35.265,161.199l-1.9,4.757c-1.34,3.343-2.478,6.802-3.586,10.268 C219.121,204.261,212.718,195.122,210.308,184.656z M288.424,147.505c-4.836,16.929-26.657,34.751-26.876,34.923 c-1.88,1.524-2.511,4.116-1.545,6.331c0.961,2.21,3.323,3.514,5.687,3.177c4.366-0.629,8.608-1.837,12.361-3.202 c-12.143,21.076-28.852,25.759-38.53,26.563c1.039-3.267,2.12-6.521,3.376-9.676l1.904-4.745 c18.232-45.44,46.882-116.934,35.403-169.667c6.985,5.008,12.886,10.002,14.781,24.822c3.753,29.373-21.89,73.423-22.146,73.864 c-1.01,1.708-0.987,3.837,0.047,5.525c1.039,1.697,2.914,2.682,4.903,2.564c5.326-0.313,10.299-3.531,14.52-7.596 C290.418,141.022,288.486,147.33,288.424,147.505z"></path> </g> </g></svg>
                                        لواز التحریر
                                    </a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/calculator">
                                                ماشین حساب
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/cutter">
                                                کاتر
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Spring-machine">
                                                دستگاه فنر زنی
                                            </a>
                                        </li>

                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>

                        </ol>
                    </div>
                    <div class="col-sm-4 col-md-3">

                    </div>
                    <div class="col-sm-4 col-md-3">
                        @if(\Illuminate\Support\Facades\Auth::check()) @endif
                    </div>
                </div>
            </div>
            <div class="tab-pane fade" id="Shop-and-bank-equipment" role="tabpanel"
                 aria-labelledby="Shop-and-bank-equipment-tab" tabindex="0">
                <!--      تجهیزات فروشگاهی و بانکی       -->
                <div class="row">
                    <div class="col-sm-4 col-md-3">
                        <ol class="list-group ">
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/Shop-and-bank-equipment"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M12.37 2.14984L21.37 5.74982C21.72 5.88982 22 6.30981 22 6.67981V9.99982C22 10.5498 21.55 10.9998 21 10.9998H3C2.45 10.9998 2 10.5498 2 9.99982V6.67981C2 6.30981 2.28 5.88982 2.63 5.74982L11.63 2.14984C11.83 2.06984 12.17 2.06984 12.37 2.14984Z" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M22 22H2V19C2 18.45 2.45 18 3 18H21C21.55 18 22 18.45 22 19V22Z" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M4 18V11" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M8 18V11" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M12 18V11" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M16 18V11" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M20 18V11" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M1 22H23" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> <path opacity="0.4" d="M12 8.5C12.8284 8.5 13.5 7.82843 13.5 7C13.5 6.17157 12.8284 5.5 12 5.5C11.1716 5.5 10.5 6.17157 10.5 7C10.5 7.82843 11.1716 8.5 12 8.5Z" stroke="#fc454a" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                                        تجهیزات فروشگاهی و بانکی
                                    </a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Cash-register">
                                                صندوق فروشگاهی
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Sales-and-accounting-software">
                                                نرم افزار فروش و حسابداری
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Card-printer">
                                                کارت پرینتر
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Czech-Perforation">
                                                پرفراژ چک
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/money-counter">
                                                پول شمار
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Banknote-test">
                                                تست اسکناس
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Mobile-printer">
                                                موبایل پرینتر
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/money-drawer">
                                                کشوی پول
                                            </a>
                                        </li>


                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>
                        </ol>
                    </div>
                </div>
            </div>
            <div class="tab-pane fade" id="Office-medical-equipment" role="tabpanel"
                 aria-labelledby="Office-medical-equipment-tab" tabindex="0">
                <!--      تجهیزات اداری-پزشکی       -->
                <div class="row">
                    <div class="col-sm-4 col-md-3">
                        <ol class="list-group ">
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                <div class="">
                                    <a href="https://mortazavistore.ir/product-category/Office-medical-equipment"
                                       class="fw-bold text-danger">
                                        <svg class="icon-svg-panel-sm" fill="#fc454a" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <g> <path d="M465.899,128C491.355,128,512,107.355,512,81.899V46.101C512,20.645,491.355,0,465.899,0H46.101 C20.645,0,0,20.645,0,46.101v35.797C0,107.355,20.645,128,46.101,128h17.901v42.667H46.101C20.645,170.667,0,191.311,0,216.768 v249.131C0,491.355,20.645,512,46.101,512h419.797C491.355,512,512,491.355,512,465.899V216.768 c0-25.457-20.645-46.101-46.101-46.101h-17.897V128H465.899z M42.667,81.899V46.101c0-1.893,1.542-3.435,3.435-3.435h419.797 c1.893,0,3.435,1.542,3.435,3.435v35.797c0,1.893-1.542,3.435-3.435,3.435h-39.23h-85.333H170.669H85.335H46.101 C44.209,85.333,42.667,83.791,42.667,81.899z M192.002,170.667V128h128v42.667H192.002z M106.669,128h42.667v42.667h-42.667V128z M469.333,216.768v249.131c0,1.893-1.542,3.435-3.435,3.435H46.101c-1.893,0-3.435-1.542-3.435-3.435V216.768 c0-1.893,1.542-3.435,3.435-3.435h39.234h85.333h170.667h85.333h39.23C467.791,213.333,469.333,214.875,469.333,216.768z M405.336,170.667h-42.667V128h42.667V170.667z"></path> <path d="M320.002,320h-42.667v-42.667c0-11.782-9.551-21.333-21.333-21.333s-21.333,9.551-21.333,21.333V320h-42.667 c-11.782,0-21.333,9.551-21.333,21.333c0,11.782,9.551,21.333,21.333,21.333h42.667v42.667c0,11.782,9.551,21.333,21.333,21.333 s21.333-9.551,21.333-21.333v-42.667h42.667c11.782,0,21.333-9.551,21.333-21.333C341.336,329.551,331.784,320,320.002,320z"></path> </g> </g> </g> </g></svg>
                                        تجهیزات فروشگاهی و بانکی
                                    </a>
                                    <ul class="list-group">
                                        <li class="list-item ">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Patient-identification-wristband">
                                                مچ بند شناسایی بیمار
                                            </a>
                                        </li>
                                        <li class="list-item">
                                            <a class="nav-link"
                                               href="https://mortazavistore.ir/product-category/Wristband-printer">
                                                پرینتر مچ بند
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <span class="badge bg-primary rounded-pill"></span>
                            </li>
                        </ol>
                    </div>
                </div>

            </div>
        </div>
    </div>
</ul>
